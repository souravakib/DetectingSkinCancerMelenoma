...
skin_df = pd.read_csv('C:/E_Drive/kaBHOOM/UPM/FYP/codes/HAM10000_metadata.csv')
...
model.save("skincancer_detection_model.h5")
...
def capture_webcam_image():
    ...
    while True:
        ret, frame = cap.read()
        cv2.imshow('Press spacebar to capture an image', frame)
...
def preprocess_webcam_image(image, img_height, img_width):
...
preprocessed_webcam_image = preprocess_webcam_image(webcam_image, img_height, img_width)

# Make predictions on the webcam image
predictions = model.predict(preprocessed_webcam_image)
